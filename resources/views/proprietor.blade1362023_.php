                                            ...
                                            <div class="col-lg-2">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text"
                                                        ...
                                                        id="per_pincode" onInput="checkPincodeLength(this)" 
                                                        data-index="{{ $index }}"
                                                        name="per_pincode[]"
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    cities.forEach(function(city) { 
                        ...
                        $(`.per_city-${index}`).append($('<option>', {
                            value: city.Name,
                            text: city.Name
                        }));
                    });
                    $(row).find(`.per_district-${index}`).val(district);
                    $(row).find(`.per_state-${index}`).val(state);
                    // Additional AJAX request for state code and country code
                    $.ajax({
                        url: "{{ route('getstatecodedetails') }}",
                        type: 'post',
                        ...
                        success: function(response) {
                            ...
                            var countryCode = response.country_code;
                            // Set state code and country code values
                            $(row).find(`.per_state_code-${index}`).val(stateCode);
                            $(row).find(`.per_country_code-${index}`).val(countryCode);
                        }
                    });
                },
                error: function(xhr, status, error) {
