                ...
                <div class="col-lg-12">
                    <form action="{{ route('add-document') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-4">
                                <div class="form-group">
                                    <label for="salary_slip" class="text-nowrap">Salary Slip:</label>
...
//                     $.each(data, function(key, item) {
//                         // Append customer option if not already selected or added
//                         if (!selectedValues[item.customer_id]) {
//                             $('#customer_id').append($('<option>', {
//                                 value: item.customer_id,
//                                 text: item.customer_name + ' (BORROWER)'
//                             }));
//                             selectedValues[item.customer_id] = true; // Mark customer as added
//                         }

//                         // Append proprietor option if it exists and not already selected or added
...
$(document).ready(function() {
            ...
            if (loanId) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data, function(key, item) {
                            var optionText = item.customer_name + ' (' + item.role + ')';
                            $('#customer_id').append($('<option>', {
                                value: item.customer_id,
                                text: optionText
                            }));
        ...
        $('#customer_id').change(function() {
            ...
            if (customerId) {
                $('#customermainnid').val(customerId); // Set customer ID in hidden field
                displaySelectedCustomerInfo(customerId);
            } else {
                $('#selected_customer_info').empty();
                $('#customermainnid').val('');
            }
        ...
        function displaySelectedCustomerInfo(customerId) {
            // Replace with your endpoint to fetch customer info
            $.ajax({
                ...
                success: function(data) {
                    if (data) {
                        var customerInfo = data.customer_name + ' (' + data.role + ')';
                        $('#selected_customer_info').text(customerInfo);
                    } else {
                        $('#selected_customer_info').empty();
                    }
                },
